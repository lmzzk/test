...
class BannerController extends AuthController
{
    ...
    public function banner_list()
    {
        ...
        $search_title   = I('search_title');//banner名称
        $banner = M('banner');
        $sql = '';
        //查询
        if (IS_GET) {
        ...
        $list = $banner->where($where)->order("addtime desc")->limit($page->firstRow . ',' . $page->listRows)->select();
        //dump($banner);
        //赋值分页输出
 ...
 //*********************************************************************************************************************************************/   
    ...
    public function banner_alter()
    {
        ...
        if($id) 
        {
            ...
            $banner  = M('banner');
            $list  = $banner->where(array('is_del'=>0,'id'=>$id))->find();
            $this->assign('info',$list);
            $this->display();
        } else {
            $this->redirect('Banner/banner_list');
            exit();
        }
    }
    ...
    //修改方法
    public function banner_alter_do()
    { 
        $data = I('');
        $banner  = M('banner');
        $data['addtime']=date("Y-m-d H:i:s",time());
