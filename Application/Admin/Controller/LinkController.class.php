...
class LinkController extends AuthController
{
    ...
    public function link_list()
    {
        ...
        $list = $link->where($where)->order("sort desc")->limit($page->firstRow . ',' . $page->listRows)->select();
   ...
   //添加
    public function link_add()
    {
        ...
        $link_list = $link->where($where)->select();
 ...
 //*********************************************************************************************************************************************/   
    ...
    public function link_alter()
    {
        ...
        if($id) 
        {
            ...
            $link  = M('link');
            $list  = $link->where(array('is_del'=>0,'id'=>$id))->find();
            ...
            $this->redirect('Menu/link_list');
            exit();
        }
    }
        //修改方法
    public function link_alter_do()
    { 
        $data = I('');
        $banner  = M('link');
        $data['addtime']=date("Y-m-d H:i:s",time());
